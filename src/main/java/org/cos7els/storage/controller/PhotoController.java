...
public class PhotoController {
    ...
    @GetMapping("/photos")
    public ResponseEntity<List<ThumbnailResponse>> getThumbnails(@AuthenticationPrincipal UserDetailsImpl userDetails) {
        return new ResponseEntity<>(photoService.getThumbnails(userDetails.getId()), HttpStatus.OK);
    }

    @GetMapping("/photo/{photoId}")
    public ResponseEntity<PhotoResponse> getPhoto(@PathVariable Long photoId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
        PhotoResponse response = photoService.getPhoto(photoId, userDetails.getId());
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @GetMapping("/photos/download")
    public ResponseEntity<byte[]> downloadPhotos(@RequestBody SelectedPhotoRequest selectedPhotoRequest, @AuthenticationPrincipal UserDetailsImpl userDetails) {
        ...
        return new ResponseEntity<>(photoService.downloadPhotos(selectedPhotoRequest, userDetails.getId()), headers, HttpStatus.OK);
    }

    @GetMapping("/photo/{photoId}/download")
    public ResponseEntity<byte[]> downloadPhoto(@PathVariable Long photoId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
    ...
    public ResponseEntity<HttpStatus> uploadPhotos(@RequestPart("files") List<MultipartFile> files, @AuthenticationPrincipal UserDetailsImpl userDetails) {
        photoService.uploadPhoto(files, userDetails.getId());
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @DeleteMapping("/photos")
    public ResponseEntity<HttpStatus> deletePhotos(@RequestBody SelectedPhotoRequest selectedPhotoRequest, @AuthenticationPrincipal UserDetailsImpl userDetails) {
        photoService.deletePhotos(selectedPhotoRequest, userDetails.getId());
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }

    @DeleteMapping("/photo/{photoId}/delete")
    public ResponseEntity<HttpStatus> deletePhoto(@PathVariable Long photoId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
