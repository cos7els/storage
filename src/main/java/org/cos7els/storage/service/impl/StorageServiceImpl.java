...
public class StorageServiceImpl implements StorageService {
    ...
    private void putObject(String bucketName, String hashedPath, InputStream in, String contentType) {
        try (in) {
            minioClient.putObject(PutObjectArgs.builder().bucket(bucketName).object(hashedPath).stream(in, in.available(), -1L).contentType(contentType).build());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private byte[] getObject(String bucketName, Photo photo) {
        byte[] object = null;
        try (InputStream in = minioClient.getObject(GetObjectArgs.builder().bucket(bucketName).object(photo.getPath()).build())) {
    ...
    private void removeObject(String bucketName, Photo photo) {
        try {
            minioClient.removeObject(RemoveObjectArgs.builder().bucket(bucketName).object(photo.getPath()).build());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//        String filename = file.getOriginalFilename();
        ...
        return String.format("%s/%s/%s", username, hash.substring(0, 2), hash.substring(2)
